...
def _table(rows, col_ratios, header_bg=NAVY):
...
# ─────────────────────────────────────────────────────────────────────────────
# Section 4: Individual Candidate Cards
# ─────────────────────────────────────────────────────────────────────────────
def _candidate_card(c: Candidate2026) -> list:
    items = []
    conv_color = {"HIGH": GREEN, "MEDIUM": TEAL, "WATCH": AMBER, "SHORT": RED}.get(c.conviction, DGRAY)
    icon = {"HIGH": "★★ TOP PICK", "MEDIUM": "★ MEDIUM CONVICTION",
            "WATCH": "◇ WATCH", "SHORT": "⚠ DELETED / AVOID"}.get(c.conviction)
    ...
    for m_label, m_val in metrics:
        is_neg = m_val.startswith("-") or "SKIP" in m_val or "▼" in m_val
        is_pos = "✓" in m_val or "▲" in m_val or ("+" in m_val and "threshold" not in m_label)
        val_color = RED if is_neg else (GREEN if is_pos else (AMBER if "threshold" in m_label and "-" in m_val else BLACK))
        met_cells.append(Table([
            [Paragraph(m_label, S("ml", fontSize=6, textColor=DGRAY, fontName="Helvetica-Bold", alignment=TA_CENTER))],
            [Paragraph(m_val, S("mv", fontSize=8, textColor=val_color, fontName="Helvetica-Bold", alignment=TA_CENTER))],
    ...
    left = Paragraph(
        f"<b>Analysis:</b><br/>{thesis_text}<br/><br/>"
        f"<b>Why 2026 specifically:</b><br/>{why_text}",
        S("lb", fontSize=7.8, leading=11))

    right_content = [
        Paragraph("Key Risks", S("rh", fontSize=9, fontName="Helvetica-Bold", textColor=RED, spaceAfter=2)),
        Paragraph(risks_text, S("rb", fontSize=7.5, leading=11)),
        Spacer(1, 0.1*cm),
        Paragraph("Verification Checklist", S("vh", fontSize=9, fontName="Helvetica-Bold", textColor=BLUE, spaceAfter=2)),
        Paragraph(check_text, S("vb", fontSize=7.5, leading=11, textColor=colors.HexColor("#1A237E"))),
    ]
    right = Table([[item] for item in right_content],
                  style=TableStyle([("TOPPADDING",(0,0),(-1,-1),0),("BOTTOMPADDING",(0,0),(-1,-1),2)]))

    body = Table([[left, right]], colWidths=[(PAGE_W-2*MARGIN)*0.52, (PAGE_W-2*MARGIN)*0.48])
    ...
    # Footer
    if c.conviction not in ("SHORT",) and c.event_type != "Deletion Risk":
        if c.conviction == "HIGH":
            entry_text = (
                f"<b>Entry signal:</b> T−45 before {c.target_review} SAR (~14 March 2026 for May SAR)  |  "
                f"<b>Verified price:</b> PLN {c.price_pln:,.2f}  |  "
                f"<b>TP exit:</b> effective date of inclusion  |  "
                f"<b>FP exit:</b> if NOT in SAR announcement, sell within 30 min  |  "
                f"<b>Historical base rate (Standard Add, 2018-2025):</b> avg +14.3% announcement-to-effective"
            )
        else:
            entry_text = (
                f"<b>Monitor for entry:</b> {c.target_review} SAR  |  "
                f"<b>Re-screen date:</b> T−45 before {c.target_review}  |  "
                f"<b>Required:</b> cap ≥ $3.3B + RS ≥ 60th pct + above 200d MA"
            )
        bg = colors.HexColor("#E3F2FD")
        border = BLUE
    else:
        entry_text = (
            f"<b>Status:</b> {c.event_type}  —  {c.target_review}  |  "
            f"<b>Action:</b> Avoid in Standard-tracking strategies  |  "
            f"<b>Post-deletion reversal:</b> stocks typically recover +5-10% in 30 days after forced selling"
        )
